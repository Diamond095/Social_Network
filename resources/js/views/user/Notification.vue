<template>
        <div class="w-full  h-full  bg-opacity-90 top-0 overflow-y-auto overflow-x-hidden  sticky-0" id="chec-div">
            <div class="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" id="notification">
                <div v-for="notification in notifications" class="2xl:w-4/12 bg-gray-50 h-screen overflow-y-auto p-8 absolute right-0">
                    <div v-if="notification.type='like'" class="w-full p-3 mt-8 bg-white rounded flex">
                        ...
                        <div class="pl-3">
                            <p tabindex="0" class="focus:outline-none text-sm leading-none"><span class="text-indigo-700">{{notification.user.name}}</span> поставил лайк вашему посту с названием <span class="text-indigo-700">"{{notification.title}}"</span></p>
